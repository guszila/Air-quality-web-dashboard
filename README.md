...
## การเชื่อมต่อข้อมูลกับ Google Sheets

แอปพลิเคชันนี้ดึงข้อมูลคุณภาพอากาศมาจาก Google Sheets เพื่อให้แอปใช้งานได้ คุณต้องนำลิงก์ (URL) ของ Google Sheets มาใส่ในโปรเจกต์ตามขั้นตอนต่อไปนี้:

**ขั้นตอนที่ 1: นำลิงก์มาจาก Google Sheets**
1. เปิดไฟล์ Google Sheets ที่มีข้อมูลเซนเซอร์ของคุณ
2. ไปที่เมนู **ไฟล์ (File)** > **แชร์ (Share)** > **เผยแพร่ทางเว็บ (Publish to web)**
3. ในหน้าต่างที่เปิดขึ้นมา ให้เปลี่ยนรูปแบบไฟล์จาก "หน้าเว็บ" เป็น **"ค่าที่คั่นด้วยจุลภาค (.csv)"**
4. กดปุ่ม **เผยแพร่ (Publish)** จากนั้น **คัดลอกลิงก์** ที่ระบบสร้างให้เตรียมไว้

**ขั้นตอนที่ 2: นำลิงก์มาใส่ในโปรเจกต์ (สร้างไฟล์ .env)**
1. ในรายชื่อไฟล์ของโปรเจกต์ ให้หาไฟล์ที่ชื่อว่า `.env.example`
2. ให้คลิกขวาที่ไฟล์ `.env.example` เลือก **Copy (คัดลอก)** แล้วกด **Paste (วาง)** ไว้ในโฟลเดอร์เดิม
3. จะได้ไฟล์สำเนาขึ้นมา ให้คลิกขวาเพื่อ **Rename (เปลี่ยนชื่อ)** ไฟล์นั้นเป็น **`.env`** (ลบคำว่า .example ออกให้หมด ให้เหลือแค่จุดแล้วตามด้วยคำว่า env)
4. ดับเบิลคลิกเปิดไฟล์ `.env` ขึ้นมา
5. นำลิงก์ที่คุณคัดลอกมาจากขั้นตอนที่ 1 ไปวางแทนที่ลิงก์เก่า โดยวางให้อยู่ในเครื่องหมายคำพูด `" "` ตัวอย่างเช่น:

```env
# ลิงก์สำหรับเซนเซอร์จุดที่ 1
VITE_GOOGLE_SHEET_URL_DEVICE1="วางลิงก์ที่คัดลอกมาตรงนี้"

# ลิงก์สำหรับเซนเซอร์จุดที่ 2
VITE_GOOGLE_SHEET_URL_DEVICE2="วางลิงก์ที่คัดลอกมาตรงนี้"
```
